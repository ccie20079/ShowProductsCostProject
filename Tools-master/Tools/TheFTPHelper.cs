...
namespace Tools
{
    ...
    public class TheFTPHelper
    {
        ...
        /// <returns></returns>
        public bool ifExistsTheFileOrDir(string ftpPath, string dirName)
        {
            bool flag = true;
            try
            {
        ...
        /// <param name="pb"></param>      
        public void DownloadFtpDirWithProgressBar(string ftpDir, string saveDir,Label lbl,ProgressBar pb)
        {
            List<FileStruct> files = ListFilesAndDirectories(ftpDir);
            if (!Directory.Exists(saveDir))
            {
            ...
            pb.Maximum = getFileCountsInFtpDir(ftpDir);
            pb.Value = 0;
            foreach (FileStruct f in files)
            {
                if (f.IsDirectory) //文件夹，递归查询  
        ...
        public void DownloadFtpDirWithBackgroudWorker(string ftpDir, string saveDir, BackgroundWorker bgWorker)
        {
            List<FileStruct> files = ListFilesAndDirectories(ftpDir);
            if (!Directory.Exists(saveDir))
            {
            ...
            foreach (FileStruct f in files)
            {
                ...
                else //文件，直接下载  
                {
                    ...
                    //     The System.ComponentModel.BackgroundWorker.WorkerReportsProgress property is
                    //     set to false.
                    bgWorker.ReportProgress(i++,f.Name);
                }
            }
        }
        /// <summary>
        ...
        /// <param name="ftpDir"></param>
        /// <returns></returns>
        public int getFileCountsInFtpDir(string ftpDir)
        {
            List<FileStruct> files = ListFilesAndDirectories(ftpDir);
            foreach (FileStruct f in files)
            {
                if (f.IsDirectory) //文件夹，递归查询  
                {
                    getFileCountsInFtpDir(ftpDir + "/" + f.Name);
                }
                else //文件，直接下载  
                {
                    numOfFiles++;
